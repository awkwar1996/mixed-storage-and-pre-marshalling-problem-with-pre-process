...
if __name__ == '__main__':
    ...
    for file_name in instances_files:
        ...
        iter_result_list, iter_relTime_list, iter_bpNum_list = ga.main(copy.deepcopy(env))
        end_time = time.time()
        save_result2(iter_result_list, iter_relTime_list, iter_bpNum_list, f'result/ga/{file_name}',
                     end_time - start_time)
        print(iter_result_list)

        del S, H, M, N_i, P, inbound_plates, preprocess_time, config, operate_time, min_operate_time
        del env, ga
        del iter_result_list, iter_relTime_list, iter_bpNum_list
